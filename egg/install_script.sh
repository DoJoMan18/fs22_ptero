...
if [ -n "${ADM_PASS}" ]; then
    sed -i "s/<passphrase>webpassword<\/passphrase>/<passphrase>${ADM_PASS}<\/passphrase>/" /mnt/server/dedicatedServer.xml
...
if [ -n "${HTTP_PORT}" ]; then
    sed -i "s/listen 1234;/listen ${HTTP_PORT};/" /mnt/server/etc/nginx/sites-enabled/fs22
fi
